...
export default function App() {
  ...
  const [isPaused, setIsPaused] = useState(false); // botón pausa
  ...
  const onDrop = (targetId) => {
    ...
    setItems((prev) =>
      prev.map((it) => (idToPos.has(it.id) ? { ...it, position: idToPos.get(it.id) } : it))
    );
    setDragId(null);
  };
  ...
  const openLightboxAt = (id) => {
    ...
    if (idx >= 0) {
      ...
      setIsPaused(visibleItems[idx]?.type === "youtube"); // autopausa si es video
    }
  };
  ...
  const uploadToCloudinary = async () => {
    try {
      ...
      if (data.secure_url) {
        ...
        alert("No se pudo subir. Revisa tu cloud/preset.");
      }
    } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white text-slate-800 dark:from-slate-900 dark:to-slate-900 dark:text-slate-100">
        ...
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Secciones, fotos y videos de YouTube
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <input
                className="input pl-9 w-64 bg-white dark:bg-slate-900"
                value={query}
            ...
            <button
              ...
              title="Subir archivo (Cloudinary)"
            >
      ...
      {/* Lightbox con Play/Pause */}
      {lightboxOpen && currentItem && (
        <div className="lightbox-backdrop" onClick={() => setLightboxOpen(false)}>
          {/* Barra superior con Pausar/Reanudar */}
          <div
            className="absolute top-4 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-white/80 dark:bg-slate-800/80 rounded-full px-3 py-1.5 shadow"
          ...
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              setIsPaused(true); // pausar al navegar manualmente
              prev();
            }}
            aria-label="Anterior"
          >
      ...
      {showAddSection && (
        ...
        >
          <div
            className="bg-white dark:bg-slate-900 rounded-2xl p-4 w-full max-w-md shadow-xl"
            onClick={(e) => e.stopPropagation()}
          >
      ...
      {showAddItem && (
        ...
        >
          <div
            className="bg-white dark:bg-slate-900 rounded-2xl p-4 w-full max-w-lg shadow-xl"
            onClick={(e) => e.stopPropagation()}
          >
            <h3 className="text-lg font-semibold mb-1">Agregar elemento</h3>
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-3">
              Sube una foto por URL o un video de YouTube.
            </p>
            ...
            <div className="grid gap-3">
              ...
              <div>
                <label className="text-sm">
                  {newType === "youtube" ? "URL de YouTube" : "URL de la imagen"}
                </label>
                <input
                  className="input bg-white dark:bg-slate-900"
                  value={newUrl}
      ...
      {showUpload && (
        ...
        >
          ...
          >
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-3">
              Usa Cloudinary (preset sin firmar).
            </p>
              ...
              <div className="text-xs text-slate-500 dark:text-slate-400">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    if (cloudName && preset) {
                      setCloudCfg({ cloudName, preset });
                      localStorage.setItem(CLOUD_KEY, JSON.stringify({ cloudName, preset }));
                    }
                  }}
                >
                  configurar
